...
public class MetaLiquidFlowing extends BlockFlowing {
	/**
	...
	protected MetaLiquidFlowing(int par1, Material par2Material) {
	...
	public void updateTick(World par1World, int par2, int par3, int par4, Random par5Random) {
		int l = getFlowDecay(par1World, par2, par3, par4);
		byte b0 = 2;
		...
		if (l > 0) {
			...
			int j1 = getSmallestFlowDecay(par1World, par2 - 1, par3, par4, b1);
			j1 = getSmallestFlowDecay(par1World, par2 + 1, par3, par4, j1);
			j1 = getSmallestFlowDecay(par1World, par2, par3, par4 - 1, j1);
			j1 = getSmallestFlowDecay(par1World, par2, par3, par4 + 1, j1);
			i1 = j1 + b0;
			...
			if (getFlowDecay(par1World, par2, par3 + 1, par4) >= 0) {
				int k1 = getFlowDecay(par1World, par2, par3 + 1, par4);
		...
		if (liquidCanDisplaceBlock(par1World, par2, par3 - 1, par4)) {
			...
			if (l >= 8) {
				flowIntoBlock(par1World, par2, par3 - 1, par4, l);
			} else {
				flowIntoBlock(par1World, par2, par3 - 1, par4, l + 8);
			}
		} else if ((l >= 0) && ((l == 0) || blockBlocksFlow(par1World, par2, par3 - 1, par4))) {
			boolean[] aboolean = getOptimalFlowDirections(par1World, par2, par3, par4);
			i1 = l + b0;
	...
	private int calculateFlowCost(World par1World, int par2, int par3, int par4, int par5, int par6) {
		...
		for (int k1 = 0; k1 < 4; ++k1) {
			if (((k1 != 0) || (par6 != 1)) && ((k1 != 1) || (par6 != 0)) && ((k1 != 2) || (par6 != 3)) && ((k1 != 3) || (par6 != 2))) {
				...
				if (!blockBlocksFlow(par1World, l1, par3, i2)
						&& ((par1World.getBlockMaterial(l1, par3, i2) != this.blockMaterial) || (par1World.getBlockMetadata(l1, par3, i2) != 0))) {
					if (!blockBlocksFlow(par1World, l1, par3 - 1, i2)) {
						return par5;
					}

					if (par5 < 4) {
						int j2 = calculateFlowCost(par1World, l1, par3, i2, par5 + 1, k1);
	...
	private boolean[] getOptimalFlowDirections(World par1World, int par2, int par3, int par4) {
		...
		for (l = 0; l < 4; ++l) {
			...
			if (!blockBlocksFlow(par1World, i1, par3, j1)
					&& ((par1World.getBlockMaterial(i1, par3, j1) != this.blockMaterial) || (par1World.getBlockMetadata(i1, par3, j1) != 0))) {
				if (blockBlocksFlow(par1World, i1, par3 - 1, j1)) {
					this.flowCost[l] = calculateFlowCost(par1World, i1, par3, j1, 1, l);
				} else {
					this.flowCost[l] = 0;
				}
			}
	...
	protected int getSmallestFlowDecay(World par1World, int par2, int par3, int par4, int par5) {
		int i1 = getFlowDecay(par1World, par2, par3, par4);
	...
	private boolean liquidCanDisplaceBlock(World par1World, int par2, int par3, int par4) {
		...
		return material == this.blockMaterial ? false : (material == Material.lava ? false : !blockBlocksFlow(par1World, par2, par3, par4));
	}
