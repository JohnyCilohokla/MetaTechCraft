...
*/
    ...
    public static class DepLoadInst {
        ...
        private void addClasspath(String name) {
            try {
                ((LaunchClassLoader) DependencyLoader.class.getClassLoader()).addURL(new File(v_modsDir, name).toURI().toURL());
        ...
        private void deleteMod(File mod) {
            ...
            try {
                ClassLoader cl = DependencyLoader.class.getClassLoader();
                ...
                Method m_get = URLClassPath.getMethod("get", ClassLoader.class);

                Class<?> URLUtil = Class.forName("sun.net.util.URLUtil");
                Method m_urlNoFragString = URLUtil.getMethod("urlNoFragString", URL.class);
                
                f_ucp.setAccessible(true);
                f_loaders.setAccessible(true);
                f_lmap.setAccessible(true);
                
                Object ucp = m_get.invoke(f_ucp, cl);

                //urlClassPath ucp = (urlClassPath) f_ucp.get(cl);
                Object t = m_urlNoFragString.invoke(null, url);
                @SuppressWarnings("unchecked")
				Closeable loader = ((Map<String, Closeable>)f_lmap.get(ucp)).remove(t);
                if (loader != null) {
                    loader.close();
...
throw new RuntimeException(String.format("The downloaded file %s has an invalid checksum %s (expecting %s). The download did not succeed correctly and the file has been deleted. Please try launching again.", target.getName(), cksum, validationHash));
        ...
        private void loadJson(JsonObject node) throws IOException {
            boolean obfuscated = ((LaunchClassLoader) DependencyLoader.class.getClassLoader())
                    .getClassBytes("net.minecraft.world.World") == null;

            String testClass = node.get("class").getAsString();
            if (DependencyLoader.class.getResource("/" + testClass.replace('.', '/') + ".class") != null)
                return;
