...
public class OreGenReplace extends OreGenBase {
	...
	public OreGenReplace(MineableBlockDescriptior ore, int replaceID, int minGenerateLevel, int maxGenerateLevel, int amountPerChunk, int amountPerBranch) {
		super(ore);
	...
	public void generate(World world, Random random, int varX, int varZ) {
		try {
			...
			System.out.println("Error generating ore: " + this.ore.name);
			e.printStackTrace();
		}
	}
